...
; tauri.conf.json) so it lands in $INSTDIR. We run it silently after install and
; then delete it. "/install /quiet /norestart" is idempotent: it is a no-op when
; an equal-or-newer runtime is already present (e.g. exit code 1638).
