...
pub async fn search(
    ...
    match query {
        SearchQuery::Text { ref query } => {
            ...
            let tag_image_ids = crate::tags::repo::get_tag_image_ids_ordered(pool, query)
                .await
                .unwrap_or_default();

            // 2. Name-derived images (accent-insensitive), appended after tag matches.
            let matched = crate::tags::repo::search_subjects_matching(pool, query)
                .await
                .unwrap_or_default();
            let subject_ids: Vec<i64> = matched.iter().map(|m| m.subject.id).collect();
            let name_image_ids = crate::tags::repo::get_image_ids_for_subjects(pool, &subject_ids)
                .await
                .unwrap_or_default();
            ...
            let query_embedding = if let Some(cached) =
                crate::search::repo::get_cached_embedding(pool, &cache_key, "text")
                    .await
                    .unwrap_or(None)
            {
                ...
                let emb = state
                    .vision_engine
                    .embed_text(&state.model_manager, query, spec)
                    .map_err(map_err)?;
                let blob = crate::search::math::f32_slice_to_bytes(&emb);
                let _ =
                    crate::search::repo::insert_cached_embedding(pool, &cache_key, "text", &blob)
                        .await;
                emb
            };
        ...
        SearchQuery::ImageId { image_id } => {
            ...
            let embedding_f32 =
                crate::search::math::bytes_to_f32_vec(&embedding_blob).map_err(map_err)?;
            ...
            search::build_search_results(pool, scored)
                .await
                .map_err(map_err)
        }

        SearchQuery::ImageBytes {
            ref data,
            mime_type: _,
        } => {
            ...
            let query_embedding = if let Some(cached) =
                crate::search::repo::get_cached_embedding(pool, &cache_key, "image")
                    .await
                    .unwrap_or(None)
            {
                crate::search::math::bytes_to_f32_vec(&cached).map_err(map_err)?
                ...
                let emb = state
                    .vision_engine
                    .embed_image(&state.model_manager, &img, spec)
                    .map_err(map_err)?;
                let blob = crate::search::math::f32_slice_to_bytes(&emb);
                let _ =
                    crate::search::repo::insert_cached_embedding(pool, &cache_key, "image", &blob)
                        .await;
                emb
            };
...
pub async fn get_processing_status(
    ...
    crate::pipeline::queue::get_processing_counts(&state.pool)
        .await
